...
const AccountEditDialog = () => {
    ...
    const { setAuth, auth, openAccountEditDialog, setOpenAccountEditDialog } = useData()
        ...
        try {
            if (profile) {
                if (profile?.type !== "image/jpeg") {
                    if (profile?.type !== "image/png") {
                        return toast.error('Only Image Files are Accepted.')
                    }
                }
                if (profile?.size > 1000000) {
                    return toast.error('Profile Should be less than 1MB')
                }
            }
            if (confirmPassword) {
            ...
            <SignUpWrapper>
                <LeftBox>
                    <label htmlFor="profile">
                        {profile ? <Image src={profile && URL.createObjectURL(profile)} alt="" /> : <Image src={auth?.profile ? auth?.profile : "/images/account.jpg"} alt="" />}
                        ...
                        {profile && profile?.type !== "image/jpeg" && profile?.type !== "image/png" && <Error>Only Image Flies .jpg, .jpeg, .png are accepted</Error>}
                    </label>
                    <Info>
