    ...
    const handleDeleteVideo = async (id) => {
        try {
            if (window.confirm('Are You Sure ?')) {
                const { data } = await axios.delete(`/api/v1/videos/delete/${id}`)
                if (data?.success) {
        ...
        <Card key={video?._id} >
            ...
            <CardContent>
                ...
                <Box>
                    ...
                    >
                        <Item onClick={() => { setOpenEditDialog(!openEditDialog); handleMenuClose() }} ><Edit />Edit Video</Item>
                        <Item onClick={() => handleDeleteVideo(video?._id)} ><Delete />Delete Video</Item>
                        <Item onClick={handleMenuClose} ><Close />Close</Item>
                    </Menu>
                    <EditVideoDialog openEditDialog={openEditDialog} setOpenEditDialog={setOpenEditDialog} video={video} />
                </Box>
                <Box style={{ display: 'flex' }}  >
