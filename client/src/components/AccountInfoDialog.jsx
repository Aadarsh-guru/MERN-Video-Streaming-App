...
const AccountInfoDialog = () => {
    ...
    const { openAccountInfoDialog, setOpenAccountInfoDialog, setAuth, auth, openAccountEditDialog, setOpenAccountEditDialog } = useData()
    ...
    return (
        <Dialog open={openAccountInfoDialog} onClose={() => setOpenAccountInfoDialog(!openAccountInfoDialog)} hideBackdrop={true} sx={{
            "& .MuiDialog-container": {
                "& .MuiPaper-root": {
                    width: "100%",
                    maxWidth: "20vw",
            ...
            {
                auth?.profile ? <Image src={auth?.profile} alt="profile" /> : <Image src='/images/account.jpg' />
            }
            {
            ...
            {
                auth && <EditButton onClick={() => { setOpenAccountEditDialog(!openAccountEditDialog); setOpenAccountInfoDialog(!openAccountInfoDialog) }} >
                    <BorderColor />
                    Edit Profile
                </EditButton>
            }
